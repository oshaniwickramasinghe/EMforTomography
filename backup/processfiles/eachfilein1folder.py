...
download_directory = "/media/oshani/Shared/UBUNTU/EMforTomography/images"
...
def plot_spectrogram_for_targeted(time, target_bin_values, label, color='blue'):
...
def mmm_over_distance(cfile_files):
    ...
    plt.title(f'Variation of Mean, Median, and Mode for {target_freq / 1e6} MHz  with Distance in feet with object 2', fontsize=14)
    plt.legend()
    plt.grid(True)

    # Save the variation plot
    variation_output_path = os.path.join(download_directory, "792_ob2_mmmofafolderwithdistance.pdf")
...
def plot_for_eachfile(cfile_files):
    ...
    for i, cfile in enumerate(cfile_files, start=1):
        ...
        plot_spectrogram_for_targeted(time, target_bin_values, label=f"{i*2} feet")  
...
mmm_over_distance(sorted_cfile_files)
